...
$(document).ready(function () {
    ...
    $("#searchBtn").click(function (event) {
        ...
        $.ajax({
            ...
            var weatherHist = $("<button>").addClass("btn btn-primary");
            var date = moment().format("lll");
            var TempF = Math.floor(response.main.temp - 273.15) * 1.80 + 32;

            //Don't have the icon working yet
            var todayIcon = $("<img>").addClass("card-icon").text(response.weather[0].icon);
            var cityDateEl = $("<h4>").addClass("card-title").text(response.name + " " + "(" + date + ")");
            var cityHistEl = $("<h3>").addClass("card-title)").text(response.name);
            ...
            //Append to history button
            $(weatherHist).append(cityHistEl);
            //Append to buttonStack div
            $("#buttonStack").append(weatherHist);
        },
